...
static STATE AverageADC(uint8_t channelNumber, uint8_t pwr2Ave, int32_t * pResult)
{
...
STATE FBREF1V8uV(uint32_t * puVolts) 
{
    int32_t osValue;

    STATE curState = AverageADC(CH_VREF1V8_FB, 6, &osValue);

    if(curState == Idle) 
    {
        *puVolts = (uint32_t) ((28125l * osValue + 16)/32);
    }
