...
#define DSPIOBJ(obj, spi, pri, LorP, regCS, maskCS) DSPIOBJ2(obj, spi, pri, LorP##regCS, maskCS)
...
/* ------------------------------------------------------------ */
    ...
    #define DSPIOBJ2(obj, spi, pri, regCS, maskCS) \
        DSPI obj((volatile uint32_t&) SPI##spi##CON, _SPI##spi##_FAULT_VECTOR, _SPI##spi##_FAULT_VECTOR, pri, (volatile uint32_t&) regCS, maskCS); \
...
private:
    ...
    } __attribute__((packed, aligned(4))) REG;
    ...
    typedef struct _SPI {
        ...
        volatile REG sxBuf;
        volatile REG sxBrg;
    } __attribute__((packed, aligned(4))) SPI;
...
//    REG&                regSS;      // register for the CS pin
    volatile uint32_t&  regSS;      // register for the CS pin
...
//        else reg.set = mask;
//    }
    void inline _setReg(volatile uint32_t& reg, uint32_t const mask, int state)
    {
        if(state == 0) reg &= ~(mask);
        else reg |= mask;
    }
...
public:
    ...
    DSPI(volatile uint32_t& spiDSPI, uint8_t irqErr, uint8_t vecErr, uint8_t pri, volatile uint32_t& regCS, uint32_t const maskCS) : 
        spi(*((SPI *) &spiDSPI)), 
    ...
    // Data transfer functions.
    void		setSelect(volatile uint32_t& regCS, uint32_t const maskCS, uint8_t sel) { _setReg(regCS, maskCS, sel); };
