...
#define LOW_HIGH_BAUD_SPLIT     9600            // no lower than 400 baud

int DMASerial::begin(unsigned long baudRate) {
    ...
    uart.sta.UTXISEL = 0b10;                                    // create an interrupt when the transmit buffer is empty
                                                                // make sure the DMA controller is on
    if(!DMACONbits.ON) DMACONbits.ON = 1;

    // set the DMA interrupt priorities
    IPCDMA.Priority = isrRxDMAPri;
    ...
    dma.con.CHPRI   = dmaRxPri;        // Give the UART the lowest priority
...
{
    if(cbBuf == 0) return(true);
    
    // see if we are done, and make sure no-one else has this DMA
    // Beware, checking CHSIRQ is for shared DMAs and may not be a general solution
    // you could block if other DMA users are not clearing CHSIRQ when done.
    if(isDMATxDone() && ((DMA *) pDMA)->econ.CHSIRQ == 0) 
    {
        uint32_t            iDMAChannel         = ((uint32_t) pDMA - (uint32_t) &DCH0CON) / sizeof(DMA);    // what channel are we on
        uint32_t            vecTxDMA            = _DMA0_VECTOR + iDMAChannel;
        uint32_t            ieifDMAReg          = vecTxDMA / 32;
        uint32_t            ipcDMAReg           = vecTxDMA / 4;
        uint32_t            ipcShiftDMA         = ((vecTxDMA % 4) * 8);
        volatile uint32_t&  ipcDMA              = *(&IPC0 + (ipcDMAReg  * (&IPC1-&IPC0)));
                
        pieDMA          = (&IEC0 + (ieifDMAReg * (&IEC1-&IEC0)));
        pifDMA          = (&IFS0 + (ieifDMAReg * (&IFS1-&IFS0)));
        ieifMaskTxDMA   = 0x1 << (vecTxDMA % 32);
        ...
        pdmaTx->con.CHPRI   = dmaTxPri;        // Give the UART the lowest priority
        ...
        // setup the interrupt
        pdmaTx->intr.CHBCIE = 1;                    // say we want a block transfer complete interrupt
        ipcDMA      &= ~(0b11111 << ipcShiftDMA);   // zero priority,       
        ipcDMA      |= (isrTxDMAPri << (ipcShiftDMA+2)); // priority isrTxDMAPri, sub priority 0, 
        (*pieDMA)   &= ~ieifMaskTxDMA;              // turn off the interrupt
        (*pifDMA)   &= ~ieifMaskTxDMA;              // clear the flag
        _SetVector(vecTxDMA, fnTxISR);              // set the ISR to the Tx DMA vector
        (*pieDMA) |= ieifMaskTxDMA;                 // turn on the interrupt
                
        // wait until we can get at least one space in the transmit buffer; so the DMA will have something to do
//        while ((uart.sta.w & _U1STA_UTXBF_MASK) != 0);
        while ((uart.sta.w & _U1STA_TRMT_MASK) == 0);       // wait until everything is clear
...
int DMASerial::read(void)
{
	...
	if (!dma.con.CHEN || dma.dptr == iRead)
	{
		return(-1);
	}
	else
...
{
	if(dma.con.CHEN) return (cbBuff + dma.dptr - iRead) % cbBuff;
    else return(0);
}

int DMASerial::peek()
{
	if (!dma.con.CHEN || dma.dptr == iRead) return(-1);
...
{
    if(dma.con.CHEN) iRead = dma.dptr;
}
...
void DMASerial::isrTx(void)
{
        (*pifDMA)   &= ~ieifMaskTxDMA;              // clear the flag
        (*pieDMA)   &= ~ieifMaskTxDMA;              // turn off the interrupt
        
        while(pdmaTx->con.CHEN);

        pdmaTx->conClr      = 0xFFFFFFFF;
        pdmaTx->econClr     = 0xFFFFFFFF;
        pdmaTx->intrClr     = 0xFFFFFFFF;       
        pdmaTx = NULL;
        _SetVector(vecTxDMA, _GEN_EXCPT_ADDR);
}