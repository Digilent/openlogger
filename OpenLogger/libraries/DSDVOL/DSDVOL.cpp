...
{
    UINT i = 0;
    UINT tStart = _ReadCoreTimer();

    do
    {
        // if we are to continue, yield
        if(i == CNTTRY) Yield(YYield); 

        // give it a few tries
        for(i=0; i < CNTTRY && (xchg_spi(0xFF) != 0xFF); i++);

     } while(i == CNTTRY && (_ReadCoreTimer() - tStart) < (500*tmsTicksPerMS));

     // keep the time to see how well the SD card is doing.
     tStart = (_ReadCoreTimer() - tStart);
     loopStats.UpdateEntry(LOOPSTATS::SDBUSYTIME, tStart);
    
    // how many long busies do we get in a row
    // assume a long busy is 1ms or more
    if(tStart > tmsTicksPerMS) cBusyInARow++;
    else 
    {
        loopStats.RecordCount(LOOPSTATS::SDBUSYCNT, cBusyInARow);
        cBusyInARow = 0; 
    }
...
int DSDVOL::rcvr_datablock (	/* 1:OK, 0:Failed */
    ...
    do
    {
        ...
        // give it a few tries
        for(i=0; i < CNTTRY && ((token = xchg_spi(0xFF)) == 0xFF); i++);

    } while(i == CNTTRY && (_ReadCoreTimer() - tStart) < (100*tmsTicksPerMS));
	...
	if (send_cmd(CMD0, 0) == 1) {			/* Enter Idle state */
		...
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			...
			if ((ocr[2] == 0x01) && (ocr[3] == 0xAA)) {				/* The card can work at vdd range of 2.7-3.6V */
                ...
                Yield(YYield);
                while (tmsTimer(1) && send_cmd(ACMD41, 0x40000000)) Yield(YYield); 	/* Wait for leaving idle state (ACMD41 with HCS bit) */
			...
			while (tmsTimer(1) && send_cmd(cmd, 0)) Yield(YYield);		/* Wait for leaving idle state */
