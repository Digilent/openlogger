...
//*     
//* ------------------------------------------------------------
#define OSSerialOBJ(obj, buff, buffSize, uart, dma, isrPri, dmaPri) OSSerialOBJ2(obj, buff, buffSize, uart, dma, isrPri, dmaPri)
...
// the actual implementation of the helper macros
    ...
    #define OSSerialOBJ2(obj, buff, buffSize, uart, dma, isrPri, dmaPri) \
        DMASerialRxISR(obj, _DMA##dma##_VECTOR, isrPri); \
        DMASerialTxISR(obj, obj##_TxDMA_ISR, isrPri); \
        OSSerial obj(buff, buffSize, &U##uart##MODE, _UART##uart##_RX_VECTOR, &DCH##dma##CON, _DMA##dma##_VECTOR, isrPri, dmaPri, obj##_TxDMA_ISR)
#else // _DMAx_IRQ _DMA_x_VECTOR _UARTx_RX_IRQ 
    #define OSSerialOBJ2(obj, buff, buffSize, uart, dma, isrPri, dmaPri) \
        DMASerialISR(obj, _DMA_##dma##_VECTOR, isrPri); \
        DMASerialTxISR(obj, obj##_TxDMA_ISR, isrPri); \
        OSSerial obj(buff, buffSize, &U##uart##MODE, _UART##uart##_RX_IRQ, &DCH##dma##CON, _DMA##dma##_IRQ, isrPri, dmaPri)
#endif
...
public:
    ...
    OSSerial(uint8_t * const pBuffP,  uint32_t const cbBuffP, volatile void * pUART, uint8_t const irqRxP, volatile void * pDMA, uint8_t const irqDMAP, uint8_t const isrPriP, uint8_t const dmaPriP, void (* fnTxISRP)(void)) :
        DMASerial(pBuffP, cbBuffP, pUART, irqRxP, pDMA, irqDMAP, isrPriP, dmaPriP, isrPriP, dmaPriP, fnTxISRP),
            printBuff((uint8_t *) _KVA2KSEG1(_printBuff)), iPrintBuffStart(0), iPrintBuffEnd(0), cbDMA(0), fEnablePrint(true) 
        {
